...
("Certify key") generated in software, and subkeys derived from the main key
deployed on the hardware security keys which are then used for regular
operations.
...
As an example of what such extra protection could be like: When the key is
...
you should keep one key in a safe location, e.g. do not keep the pair
...
could easily be used to extend its own expiration date. However, on subkeys this
can be very effective and is recommended.
The use case as {term}`signing keys <Signing Key>` is point-in-time action
anyway, but if subkeys are used for encryption/decryption communication, for
example in confidential mail exchanges - then you would usually want to be able
to use that subkey for as long as you want/need to be able to decrypt whatever
was encrypted by others for you.
If a subkey used for such communication has expired but is still
...
revocation certificate. To be able to do so exactly when it is needed, it can be
...
{term}`Signing Key`. It identifies the developer as an individual - not as a
role, an Ubuntu team membership, or a company team membership.

Therefore the goal for the developer is to be in sole control and ownership of
the hardware key, to have sole knowledge of the passphrase, and sole control of the systems it is
plugged into. Hardware keys are beneficial in this case as they can help to
avoid exfiltration if compromised and furthermore allow separating ownership
...
unavailable to you, what then? In that case it is worth thinking about
...
security team), Technical Board (does it fit the project?), technical authors (to ensure
...
There isn't yet a defined set of requirements that such an alternative recommendation
would need to fulfil to be considered as trusted. The first to be processed
will need to identify that through discussion and collaboration. Initial
